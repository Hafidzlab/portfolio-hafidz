...
export default function SplashScreen({
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-black">
      {/* background glow (match hero: sky/blue/cyan) */}
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -right-40 top-40 h-[520px] w-[520px] rounded-full bg-sky-500/18 blur-3xl" />
        <div className="absolute -left-40 bottom-40 h-[520px] w-[520px] rounded-full bg-cyan-500/14 blur-3xl" />
      </div>
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <div className="relative mb-8 h-44 w-44">
          ...
          {/* rotating gradient ring (sky/blue/cyan) */}
          <div className="absolute inset-0 rounded-full splash-rotate">
            <div className="absolute inset-0 rounded-full bg-gradient-to-r from-sky-400 via-blue-500 to-cyan-300 opacity-90 blur-[0.2px]" />
            <div className="absolute inset-[10px] rounded-full bg-black" />
          </div>

          {/* orbit dots (match palette, no purple) */}
          <div className="absolute inset-0 splash-orbit">
            <span className="absolute left-1/2 top-3 h-4 w-4 -translate-x-1/2 rounded-full bg-sky-300 shadow-[0_0_18px_rgba(125,211,252,0.55)]" />
            <span className="absolute right-6 top-1/2 h-4 w-4 -translate-y-1/2 rounded-full bg-cyan-300 shadow-[0_0_18px_rgba(34,211,238,0.55)]" />
          </div>
        </div>

        {/* Title: same vibe as Hello World */}
        <h1 className="text-5xl md:text-6xl font-extrabold bg-gradient-to-r from-sky-300 via-blue-400 to-cyan-300 bg-clip-text text-transparent">
          {title}
        </h1>

        <p className="mt-3 text-white/45 text-base md:text-lg">{subtitle}</p>

        {/* progress bar (match hero palette) */}
        ...
        {/* big subtle orbit ring behind (cool tones, not purple) */}
        <div className="pointer-events-none absolute right-[8%] top-[52%] hidden md:block">
          <div className="relative h-[420px] w-[420px] splash-rotate-slow opacity-35">
            <div className="absolute inset-0 rounded-full ring-2 ring-sky-400/20" />
